          ...
          <div className="card p-8">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div>
                ...
                <input type="email" required value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })}
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPass ? "text" : "password"} required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                    placeholder="Nhập mật khẩu" className="input-field pr-12" />
                  <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted hover:text-brand-blue transition-colors">
                    <span className="material-symbols-outlined" style={{ fontSize: 20 }}>{showPass ? "visibility_off" : "visibility"}</span>
                  </button>
                </div>
              </div>
              <button type="submit" disabled={loading} className="btn-primary w-full py-3.5 rounded-xl text-base disabled:opacity-60 mt-1 active:scale-95 transition-transform">
                {loading ? (
                  <><span className="material-symbols-outlined animate-spin" style={{ fontSize: 18 }}>progress_activity</span> Đang đăng nhập...</>
                ) : "Đăng nhập"}
              </button>
            </form>
            ...
            <p className="text-center text-sm text-text-muted mt-6">
              Chưa có tài khoản?{" "}
              <Link to="/register" className="text-brand-blue font-bold hover:underline">Đăng ký miễn phí</Link>
            </p>
          </div>
        </div>
      </div>
